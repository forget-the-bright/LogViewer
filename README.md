...
- **主题**：自动跟随系统（`prefers-color-scheme`）/ 明 / 暗三态，xterm 同步切换，选择持久化。
- **键盘快捷键**：`/` 搜索、`t` 跟踪/静态、`s` 开始/停止、`g`/`G` 顶/底、`PgUp`/`PgDn` 翻页、
  `?` 快捷键帮助、`Ctrl+F` 终端内搜索、`Ctrl+Shift+P` 命令面板（子序列模糊搜索所有操作）；
  输入框聚焦时不误触。
- **国际化与密度**：中 / 英双语切换，紧凑 / 舒适密度（字号、行高）可调，设置全部持久化。
- **移动端适配**：窄屏汉堡菜单、侧栏全屏覆盖、配置面板底部抽屉。
...
- [SSH 远程与登录设计](docs/SSH远程与登录设计.md) —— 设计记录（多机器/SSH 与登录认证均已实现）
